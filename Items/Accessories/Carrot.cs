...
namespace GiuxItems.Items.Accessories
{
    public class Carrot : ModItem
    {
        ...
        {
            KentusDashPlayer mp = player.GetModPlayer<KentusDashPlayer>();
            ...
            Dust.NewDust(player.position, player.width, player.height, 29);

            //If the dash has just started, apply the dash velocity in whatever direction we wanted to dash towards
            if (mp.DashTimer == KentusDashPlayer.MAX_DASH_TIMER)
            {
                Vector2 newVelocity = player.velocity;

                //Only apply the dash velocity if our current speed in the wanted direction is less than DashVelocity
                if ((mp.DashDir == KentusDashPlayer.DashUp && player.velocity.Y > -mp.DashVelocity) || (mp.DashDir == KentusDashPlayer.DashDown && player.velocity.Y < mp.DashVelocity))
                {
                    //Y-velocity is set here
                    //If the direction requested was DashUp, then we adjust the velocity to make the dash appear "faster" due to gravity being immediately in effect
                    //This adjustment is roughly 1.3x the intended dash velocity
                    float dashDirection = mp.DashDir == KentusDashPlayer.DashDown ? 1 : -1.3f;
                    newVelocity.Y = dashDirection * mp.DashVelocity;
                }
                else if ((mp.DashDir == KentusDashPlayer.DashLeft && player.velocity.X > -mp.DashVelocity) || (mp.DashDir == KentusDashPlayer.DashRight && player.velocity.X < mp.DashVelocity))
                {
                    //X-velocity is set here
                    int dashDirection = mp.DashDir == KentusDashPlayer.DashRight ? 1 : -1;
            ...
            if (mp.DashDelay == 0)
            {
                ...
                mp.DashDelay = KentusDashPlayer.MAX_DASH_DELAY;
                mp.DashTimer = KentusDashPlayer.MAX_DASH_TIMER;
                mp.DashActive = false;
            }
        }
    ...
    public class KentusDashPlayer : ModPlayer
    {
